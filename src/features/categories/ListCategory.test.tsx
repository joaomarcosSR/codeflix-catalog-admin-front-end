...
export const handlers = [
  ...
  rest.delete(
    `${baseUrl}/categories/${categoryResponse.items[0].id}`,
    (_, res, ctx) => {
...
describe("List category", () => {
  ...
  it("should render success state", async () => {
    renderWithProviders(<CategoryList />);
    await waitFor(() => {
      const name = screen.getByText(categoryResponse.items[0].name);
  ...
  it("should handle on page change", async () => {
    ...
    await waitFor(() => {
      const name = screen.getByText(categoryResponsePage2.items[0].name);
      expect(name).toBeInTheDocument();
    });
  });
  ...
  it("should handle delete category success", async () => {
    ...
    await waitFor(() => {
      const notification = screen.getByText("Category deleted successfully");
  ...
  it("should handle delete category error", async () => {
    ...
    await waitFor(() => {
      const notification = screen.getByText("Category not deleted");
      expect(notification).toBeInTheDocument();
    });
  });
});
